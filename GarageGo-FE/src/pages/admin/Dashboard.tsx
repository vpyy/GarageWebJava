...
export const AdminDashboard: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      'Chờ thanh toán': { bg: '#fee2e2', color: '#991b1b', text: 'Chờ thanh toán' }
    };
    const config = statusConfig[status as keyof typeof statusConfig] || statusConfig['Đang xử lý'];
  ...
  if (!dashboardData) {
    return (
      <div className="text-center py-20">
        ...
        <h3 className="text-xl font-semibold text-gray-800 mb-2">Không thể tải dữ liệu</h3>
        <button
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-100 text-sm font-medium">Doanh thu tháng</p>
              <p className="text-2xl font-bold">{formatCurrency(dashboardData.doanhThuThangNay)}</p>
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{dashboardData.tongKhachHang.toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-orange-100 text-sm font-medium">Tổng dịch vụ</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border">
          ...
          {dashboardData.doanhThuTheoThang && dashboardData.doanhThuTheoThang.length > 0 ? (
            <div className="flex items-end justify-between h-64 gap-2">
              {dashboardData.doanhThuTheoThang.map((item, index) => {
                const maxDoanhThu = Math.max(...dashboardData.doanhThuTheoThang.map(x => x.doanhThu));
              ...
              dashboardData.topSanPham.slice(0, 5).map((product, index) => (
                <div key={product.maSP} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-800 text-sm">{product.tenSP}</p>
                      <p className="text-xs text-gray-500">{product.soLuong} đã bán</p>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Hóa đơn gần đây</h3>
            <button className="text-blue-500 hover:text-blue-600 text-sm font-medium">
              Xem tất cả →
            </button>
          </div>
          
          {dashboardData.hoaDonGanDay && dashboardData.hoaDonGanDay.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left py-2 text-xs font-medium text-gray-500 uppercase">Mã HĐ</th>
                    <th className="text-left py-2 text-xs font-medium text-gray-500 uppercase">Khách hàng</th>
                    <th className="text-left py-2 text-xs font-medium text-gray-500 uppercase">Biển số</th>
                    <th className="text-right py-2 text-xs font-medium text-gray-500 uppercase">Tổng tiền</th>
                    <th className="text-center py-2 text-xs font-medium text-gray-500 uppercase">Trạng thái</th>
                  </tr>
                </thead>
                <tbody>
                  {dashboardData.hoaDonGanDay.slice(0, 6).map((invoice) => (
                    <tr key={invoice.maHD} className="border-b border-gray-50">
                      <td className="py-3 text-sm font-medium text-blue-600">#{invoice.maHD}</td>
                      <td className="py-3 text-sm text-gray-800">{invoice.tenKH}</td>
                      <td className="py-3 text-sm text-gray-600 font-mono">{invoice.bienSo}</td>
        ...
        {/* Top Services */}
        <div className="bg-white rounded-2xl p-6 shadow-sm border">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Dịch vụ phổ biến</h3>
          <div className="space-y-4">
            {dashboardData.topDichVu && dashboardData.topDichVu.length > 0 ? (
              dashboardData.topDichVu.slice(0, 5).map((service, index) => (
                <div key={service.maDV} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-800">{service.tenDV}</p>
                      <p className="text-sm text-gray-500">{service.soLanSuDung} lần sử dụng</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold text-green-600">{formatCurrency(service.doanhThu)}</p>
                    <p className="text-xs text-gray-500">{service.soLuong} dịch vụ</p>
                  </div>
                </div>
              ))
            ) : (
